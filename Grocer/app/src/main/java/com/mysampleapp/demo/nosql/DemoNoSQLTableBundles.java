...
public class DemoNoSQLTableBundles extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableBundles.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKey extends DemoNoSQLOperationBase {
        ...
        public boolean executeOperation() throws AmazonClientException {
            ...
            result = mapper.load(BundlesDO.class, AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLBundlesResult(result));
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<BundlesDO> results;
        private Iterator<BundlesDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            results = mapper.scan(BundlesDO.class, scanExpression);
            if (results != null) {
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<BundlesDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final BundlesDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLBundlesResult(item));
            itemsRetrieved++;
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        final BundlesDO firstItem = new BundlesDO();

        firstItem.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
        firstItem.setName(
            DemoSampleDataGenerator.getRandomSampleString("name"));
        firstItem.setRecipeDescription(
            DemoSampleDataGenerator.getRandomSampleString("recipe_description"));
        firstItem.setRecipeItems(DemoSampleDataGenerator.getSampleList());
        firstItem.setRecipes(DemoSampleDataGenerator.getSampleMap());
        ...
        final BundlesDO[] items = new BundlesDO[SAMPLE_DATA_ENTRIES_PER_INSERT-1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT-1; count++) {
            final BundlesDO item = new BundlesDO();
            ...
            item.setRecipeDescription(DemoSampleDataGenerator.getRandomSampleString("recipe_description"));
            item.setRecipeItems(DemoSampleDataGenerator.getSampleList());
            item.setRecipes(DemoSampleDataGenerator.getSampleMap());
    ...
    public void removeSampleData() throws AmazonClientException {
        ...
        final BundlesDO itemToFind = new BundlesDO();
        itemToFind.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());

        final DynamoDBQueryExpression<BundlesDO> queryExpression = new DynamoDBQueryExpression<BundlesDO>()
            .withHashKeyValues(itemToFind)
            .withConsistentRead(false)
            .withLimit(MAX_BATCH_SIZE_FOR_DELETE);

        final PaginatedQueryList<BundlesDO> results = mapper.query(BundlesDO.class, queryExpression);

        Iterator<BundlesDO> resultsIterator = results.iterator();
        ...
        final List<BundlesDO> batchOfItems = new LinkedList<BundlesDO>();
